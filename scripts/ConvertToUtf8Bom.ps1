...
function Test-IsBinaryFile {
    ...
    try {
        ...
        # UTF-16/UTF-32 BOMが検出された場合はテキストとみなす
        if ($bytesRead -ge 4 -and $buffer[0] -eq 0xFF -and $buffer[1] -eq 0xFE -and $buffer[2] -eq 0x00 -and $buffer[3] -eq 0x00) { return $false }
        if ($bytesRead -ge 4 -and $buffer[0] -eq 0x00 -and $buffer[1] -eq 0x00 -and $buffer[2] -eq 0xFE -and $buffer[3] -eq 0xFF) { return $false }
        if ($bytesRead -ge 2 -and $buffer[0] -eq 0xFF -and $buffer[1] -eq 0xFE) { return $false }
        if ($bytesRead -ge 2 -and $buffer[0] -eq 0xFE -and $buffer[1] -eq 0xFF) { return $false }
...
function Get-FileContentWithEncoding {
    ...
    try {
        ...
        return [PSCustomObject]@{ Content = $content; Encoding = $utf8NoBom }
    }
...
function Convert-LineEndingsToCrlf {
    ...
    $files = Get-ChildItem -Path $RootPath -Recurse -File -Force |
        ...
        Where-Object { $_.Name -notin $skipFileNames }
